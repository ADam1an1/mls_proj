...
#SBATCH --time=00:20:00              # Time limit per task
#SBATCH --mem=16G                     # Memory per task
#SBATCH --array=0-10             # Array range (x jobs total)
...
echo "Running MLS_full_ss.bash with Lambda: $LAMBDA_VAL"
...
echo $LAMBDA_VAL
bash MLS_full_ss.bash $LAMBDA_VAL
