    ...
    **note that the trials are not checked for params** 
    ...
    with h5py.File(file_path, 'r') as f:
        for trial in trial_names:
            data.append(f[trial_name + "/" + trial_data][:])
...
def run_trial(file_path, trial_name, params, operators):
    ...
    with h5py.File(file_path, 'a') as f:
        ...
        if not check_trial(f, trial_name, params, operators):
            ...
            result = run_simulation(system_e_levels=[params['system_e_level'] for i in range(params["N"])],
            ...
            if "photons" in operators: 
                trial_group.create_dataset("photon_number", data=result.expect[shift:len(params['photon_freqs']) + shift])
